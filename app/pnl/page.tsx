...
export default async function PnlPage() {
  ...
  const realized: BucketInput[] = [
    ...
    { key: "stock", label: "Stocks", items: stockF.closed.filter((r) => showAll || !r.accountId || r.accountId === id).map((r) => ({ pnl: r.realizedPnl, date: r.closedAt, sym: r.symbol, strikeLabel: `${px(r.avgOpen)} → ${px(r.avgClose)}${r.manualBasis && !r.manualEntry ? " · cost entered by hand" : ""}`, openedAt: r.openedAt, daysHeld: r.daysHeld })) },
  ];
